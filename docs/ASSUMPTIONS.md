...
| A-038 | The provisioned Kalshi **demo** API key (`pma-kalshi-demo-api-key-id`) is authorised for portfolio **reads** only, not order entry. A minimal controlled demo order (1 contract, `yes` bid @ $0.01, `post_only`, empty-book market) could not be placed: `POST /portfolio/events/orders` → **404 `user_not_found`** ("Exchange user not found … Exchange Sharding"), while every `GET /portfolio/*` on the same key returns 200. The legacy `POST /portfolio/orders` path is separately retired (**410 `deprecated_v1_order_endpoint`**). | OBSERVED 2026-09-08 (K-TR-OBS-11 / K-TR-OBS-12; `docs/evidence/kalshi-demo/lifecycle/`). Consequence: OBSERVED evidence for create-order request/response (K-TR-05/06), idempotency 409 (K-TR-07), order status (K-TR-08), fills (K-TR-09) and cancel (K-TR-10) is **blocked** until an Exchange-sharded demo account exists (account configuration — out of scope here) or a later capture. No order, position, or fill was created. Extends, does not resolve, A-037 / D-023. | Yes |
