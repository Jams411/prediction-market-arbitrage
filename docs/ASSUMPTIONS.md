...
| A-028 | Kalshi `orderbook_delta.delta_fp` is applied **additively** to the aggregated contract count already resting at that `(side, price_dollars)` level; a level reaching exactly zero is removed; a result below zero is a desync. | **OBSERVED 2026-09-09 (narrow) — was UNVERIFIED.** A production live-book session run through the shipped `LiveBookConnection`/`LiveBookFeed` (`scripts/observe_kalshi_livebook_runtime.py`) compared each APPLIED delta's `(side, price)` book quantity immediately before and after: `qty_after == qty_before + delta_fp` held for **34/34** checked deltas, including **6** where the sum reached exactly 0 and the level was correctly removed, with **0** `NEGATIVE_QUANTITY`/`CROSSED`/`SEQUENCE_GAP` outcomes across ~51 real deltas (K-LB-OBS-06; evidence `docs/evidence/kalshi-live/livebook-runtime/SUMMARY.json`, raw quantities not persisted). Still one market / one session — **OBSERVED, not VERIFIED**; a wider capture (multiple markets, a below-zero desync, a partial-fill burst) would strengthen it. | Yes |
...
| A-030 | The authenticated WebSocket handshake, subscribe body, and frame formats built by `livebook.ws_auth` and carried by `livebook.ws_transport.WebsocketsTransport` match what the real Kalshi / Polymarket US servers require. | **PARTLY RESOLVED 2026-09-09 — Kalshi half OBSERVED; Polymarket US half still UNVERIFIED.** *Kalshi:* a one-off `scripts/observe_kalshi_prod_ws_market_data.py --observe` run (operator-created production key; `scripts.kalshi_signer.OpensslRsaPssSigner`) connected to `wss://external-api-ws.kalshi.com/trade-api/ws/v2`, the real server accepted the RSA-PSS handshake + the `subscribe` body, and returned `subscribed` + `orderbook_snapshot` + one `orderbook_delta` whose decoded shapes match K-WS-03 / K-WS-04; per-subscription `seq` ran 1→2; clean disconnect → reconnect → resubscribe delivered a fresh snapshot (K-WS-OBS-01..07; evidence `docs/evidence/kalshi-live/ws-market-data/SUMMARY.json`). *Polymarket US:* still docs-only (WS-A-S4..S6), no live handshake, plus the unresolved subscribe casing/enum discrepancy between two doc pages (P-WS-AUTH-03). **Extended 2026-09-09** by a second run through the full shipped runtime (`LiveBookConnection` + `LiveBookFeed`, `scripts/observe_kalshi_livebook_runtime.py`): a sustained `seq`-ordered delta stream (30 in ~14 s, 0 desyncs), `HealthStatus` transitions `UNINITIALIZED→HEALTHY→STALE→DISCONNECTED→RESYNCING→HEALTHY`, and reconnect + REST resync — all OBSERVED against production (K-LB-OBS-01..12). *Polymarket US:* still docs-only (WS-A-S4..S6), no live handshake, plus the unresolved subscribe casing/enum discrepancy between two doc pages (P-WS-AUTH-03). **A-028** is now separately OBSERVED (see its row). | Yes |
...
- `livebook.ws_transport.WebsocketsTransport` is the concrete transport over the
  ...
  local `websockets` loopback server. A reusable caller-side Kalshi `Signer`
  (`scripts/kalshi_signer.OpensslRsaPssSigner`, `openssl` subprocess) keeps
  `cryptography` out of the package tree. **2026-09-09:** used for a real
  production Kalshi WS session (A-030 Kalshi half now OBSERVED — K-WS-OBS-01..08);
  Polymarket US still has no authenticated connection to a real venue.
